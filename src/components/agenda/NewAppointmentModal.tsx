...
export const NewAppointmentModal: React.FC<Props> = ({ initialBoxId, initialTime, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-graphite-900/50 backdrop-blur-sm flex items-end sm:items-center justify-center p-0 sm:p-4 overflow-y-auto">
      <div className="bg-white rounded-t-3xl sm:rounded-3xl max-w-lg w-full shadow-soft-lg border-t sm:border border-rose-gold-200 overflow-hidden flex flex-col max-h-[94vh] sm:max-h-[90vh] animate-scale-up">
        {/* Header */}
        <div className="p-4 sm:p-5 bg-gradient-to-r from-rose-gold-600 to-rose-gold-500 text-white shrink-0">
          <div className="w-12 h-1 bg-white/30 rounded-full mx-auto mb-2.5 sm:hidden" />
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2.5">
              <Sparkles className="w-5 h-5" />
              <h2 className="font-serif font-bold text-base sm:text-lg">Agendar Nuevo Turno</h2>
            </div>
            <button
              onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto flex flex-col bg-silk-50/50">
          ...
          <div className="sticky bottom-0 bg-white/95 backdrop-blur-sm p-3.5 sm:p-4 border-t border-rose-gold-100 flex items-center justify-end gap-2.5 shadow-soft z-10 shrink-0">
            <button
              type="button"
              onClick={onClose}
      ...
      {showQuickCreate && (
        <div className="fixed inset-0 z-[60] bg-graphite-900/50 backdrop-blur-sm flex items-end sm:items-center justify-center p-0 sm:p-4">
          <div className="bg-white rounded-t-3xl sm:rounded-3xl w-full max-w-sm shadow-soft-lg border-t sm:border border-rose-gold-200 overflow-hidden animate-scale-up">
            {/* Header */}
